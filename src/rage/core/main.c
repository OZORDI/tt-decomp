 ...
 *
 * All eight functions that carry execution from CRT startup through
 ...
 *
 * rage_Main makes five virtual calls on the memory-tracker object that
 ...
 * memory-tracking features.  Each is the address of an RTTI TypeDescriptor
 * for one of the gdUnlockCondition* classes; the check reads offset +4
 * (the TypeDescriptor's mangled-name pointer), which is always non-null
 * once the CRT has set up RTTI.  Every gate therefore fires on every run
 * of this shipped build.
 *
 ...
 *   g_bAppStarted          @ 0x825EBCA1  — set to true by rage_AppInit
...
/* String utilities used in arg parsing */
...
extern const char  k_winsock_bad_ver[];   /* "InitWinSock - Couldn't find a usable WinSock DLL (ver=%x)" @ 0x8205ADC4 */
...
typedef struct CmdArgRecord {
    ...
    int                  m_bPreset;    /* +8  nonzero = already set, skip match */
    struct CmdArgRecord* m_pNext;      /* +12 next node in singly-linked list   */
} CmdArgRecord;

extern CmdArgRecord* g_pCmdArgRegistry;  /* @ 0x825EBCF4 */

/* ── FeatureGate — RTTI TypeDescriptor used as a presence check ─────── */

typedef struct FeatureGate {
    void*        m_pVFTable;   /* +0  TypeDescriptor pVFTable (NULL in static data)  */
    const char*  m_pName;      /* +4  mangled-name ptr — non-null = feature present  */
} FeatureGate;
...
extern FeatureGate g_gate_appStarted;      /* @ 0x825CAEA4 */
...
/* ═══════════════════════════════════════════════════════════════════════════
 * netHardware_XNetStartup @ 0x82481C60 | size: 0xC
 *
 ...
 *
 * Initialises WinSock via WSAStartup.  Returns 0 on success.
 * ═══════════════════════════════════════════════════════════════════════════ */
int rage_WSAStartup(int version, void* wsaDataOut)
{
...
/* ═══════════════════════════════════════════════════════════════════════════
 * rage_ParseCommandLine @ 0x821867F8 | size: 0x178
 *
...
/* ═══════════════════════════════════════════════════════════════════════════
 * rage_AppInit @ 0x82186418 | size: 0x20C
 *
 ...
 * Named options are compared case-insensitively (via rage_StringCompareNoCase).
 ...
 * Unknown options are logged via rage_DebugLog (no-op in release).
 * ═══════════════════════════════════════════════════════════════════════════ */
void rage_AppInit(int argc, const char** argv)
{
    g_bAppStarted = true;
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        char prefix = *arg;
        bool hasPrefix = (prefix == '-') || (prefix == '/') || (prefix == (char)0x96);
        ...
        for (CmdArgRecord* node = listHead; node; node = node->m_pNext) {
            ...
            if (!eq) {
                 ...
                 * If it doesn't start with a prefix char, use it as the value. */
                const char* nextValue = NULL;
                if (i + 1 < argc) {
                    const char* next = argv[i + 1];
                    if (next) {
                        char np = *next;
                        bool nextIsOption = (np == '-') || (np == '/')
                                            || (np == (char)0x96);
                        if (!nextIsOption)
                            nextValue = next;
                    }
...
/* ═══════════════════════════════════════════════════════════════════════════
 * rage_GetExeName @ 0x82186628 | size: 0x70
 *
 * Retrieves a short exe-name token from the gate's m_pName field.
 ...
 * ═══════════════════════════════════════════════════════════════════════════ */
static void rage_GetExeName(FeatureGate* pGate, const char** outName)
{
    const char* rawName = pGate->m_pName;
    if (!rawName || !*rawName) {
        *outName = NULL;
        return;
    }
 ...
 *
 * Checks g_gate_appStarted.m_pName:
 *   - Non-null (always in this build): return true.
 *   - Null: compare g_pExeName against "A:" (Xbox 360 disc prefix).
 *           If the exe lives on disc, return true; otherwise false.
 * ═══════════════════════════════════════════════════════════════════════════ */
bool rage_IsAppStarted(void)
{
    if (g_gate_appStarted.m_pName != NULL)
        return true;

    /* Gate not active: check if exe resides on disc (path starts with "A:"). */
...
/* ═══════════════════════════════════════════════════════════════════════════
 * rage_RunMain @ 0x82186780 | size: 0x48
 *
...
int rage_Main(int argc, const char** argv)
{
    ...
    if (g_rage_main_call_depth <= 1) {
        ...
        int wsaErr = rage_WSAStartup(2, wsaData);
        if (wsaErr != 0)
            rage_DebugLog(k_winsock_no_dll);
        ...
        if (wsMajor != 2 || wsMinor != 0) {
            rage_WSACleanup();
            rage_DebugLog(k_winsock_bad_ver,
...
// ── Entry point wrapper ─────────────────────────────────────────────────────
// entry.c calls rage_main_stub(void*, void*) but the real impl is rage_Main(int, char**)
// For now, just call rage_Main with argc=0, argv=NULL since the game doesn't use cmdline
int rage_main_stub(void* pStartupParms, void* pBase) {
    (void)pStartupParms;
    (void)pBase;
