...
/**
 * UnloadTemplate @ 0x820F8EE8 | size: 0xC8
 * 
 * Unloads a template from the global template registry.
 * 
 * Removes a template entry by name from the registry array.
 * Called during object cleanup to unregister loaded templates.
 * 
 * @param templateName Template name string to unload
 */
void UnloadTemplate(const char* templateName) {
    // Find template index in registry by name
    int16_t templateIndex = game_94F0(&g_templateRegistry, (void*)templateName);
 ...
 *      - If primary object exists: call game_8EE8, then ReleaseObjectWithFlags with flag 0xE001
    ...
    if (templateIndex < (int32_t)count) {
        // Shift all entries after templateIndex down by one slot
        for (int32_t i = templateIndex; i < (int32_t)count - 1; i++) {
            g_templateRegistry.entries[i] = g_templateRegistry.entries[i + 1];
        }
    }
    
    // Decrement count
    g_templateRegistry.count = count - 1;
}
...
void game_8EE8(void* obj) {
    UnloadTemplate((const char*)obj);
            ...
            ReleaseObjectWithFlags(subObj->m_pObject1, 0xE001);
extern uint32_t g_plrPlayerMgr_state;  // @ 0x82066430
 ...
 * that were loaded for game objects. It finds the template by key,
 * removes it from the array by shifting remaining elements, and
 * decrements the count.
 * 
 * The template registry is a global array at 0x82607C40 with:
 * - 8-byte entries (2 x uint32_t fields)
 * - Count stored at offset +0x10000
 * 
 * Algorithm:
 * 1. Find template index via game_94F0
 * 2. If not found (-1), log error
 * 3. If found and not last element, shift remaining entries down
 * 4. Decrement count
 * 
 * Called by:
...
struct TemplateRegistry {
    ...
    TemplateEntry entries[8192];  // Array of template entries (8 bytes each)
    uint32_t count;               // Number of active templates (at offset +0x10000)
};

extern TemplateRegistry g_templateRegistry;  // @ 0x82607C40 (SDA +31808)
// Global state
extern uint32_t g_plrPlayerMgr_state;  // @ 0x82066430
