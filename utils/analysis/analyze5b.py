...
def fmt(c,ln,note):
    ls=(ln+':') if ln else ''
    return '  '+ls.ljust(26)+'0x'+format(c,'08X')+':  '+note

import sys
if len(sys.argv) > 1:
    target_addr = int(sys.argv[1], 16)
    insns = disasm(target_addr, 100)
    print(f'\n=== Disassembly of 0x{target_addr:08X} ===')
    for item in insns:
        print(fmt(*item))
else:
    print("Usage: python3 analyze5b.py 0xADDRESS")

# callers
def find_callers(target_va, label):
    results = []
    for si in range(num_sects):
        s=data[sect_tbl+si*40:sect_tbl+si*40+40]
        ...
        for i in range(0,vsize-4,4):
            ...
            if (w>>26)&0x3F==18 and (w&1):
                ...
                if tgt==target_va:
                    ...
                    results.append((fn_name, fn_va, cur_va))
    print('\n=== Callers of '+label+' ===')
    for fn_name,fn_va,call_va in results:
        print('  '+fn_name+' @ 0x'+format(fn_va,'08X')+'  (call at 0x'+format(call_va,'08X')+')')

find_callers(sym_by_name['grc_B750'], 'grc_B750')
find_callers(sym_by_name['rage_render_default'], 'rage_render_default')
