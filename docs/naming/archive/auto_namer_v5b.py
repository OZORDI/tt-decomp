#!/usr/bin/env python3
"""
auto_namer_v5b.py — fast single-pass string scan + forward+reverse propagation
"""
...
def propose(fn_va,name,reason,tag):
    ...
    new_names[fn_va]=name; confidence[fn_va]=reason; pass_tag[fn_va]=tag
    used_names.add(name); return True
...
pending={}  # reg -> (hi, offset_in_text)
hits=[]     # (caller_va, cls, meth)
for i in range(0,tvsize,4):
    ...
    if op==15 and ra==0:
        ...
        # Any other write to rd clears it
...
print(f"  P1: {p1} new, P1r: {p1r} partial  (total hits: {len(hits)})")
...
print("\n=== PASS 8: Forward + Reverse propagation ===")
...
for iteration in range(40):
    ...
    for fn_va in unnamed:
        ...
        elif cnt==1 and fn_meta.get(fn_va,0)<=64:
...
# ── PASS 10: Stubs ────────────────────────────────────────────────────────────
BLR=0x4E800020; TRAP=0x7FE00008; NOP=0x60000000; p10=0
for fn_va in unnamed:
    if fn_va in new_names: continue
    ...
    if real==[BLR]:
        if propose(fn_va,safe_name(f"stub_{fn_va&0xFFFF:04X}"),"blr stub","P10"): p10+=1
    elif TRAP in words:
        if propose(fn_va,safe_name(f"pure_{fn_va&0xFFFF:04X}"),"pure virtual","P10"): p10+=1
...
with open(REPORT,'w') as f:
    f.write("auto_namer_v5b Results\n"+"="*70+"\n\n")
    for tag in ['P1','P1r','P2','P6','P7','P8f','P8fw','P8r','P8rh','P9','P10']:
