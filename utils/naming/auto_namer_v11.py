#!/usr/bin/env python3
"""
auto_namer_v11.py — Extended BFS transparency + RTTI vtable completor + spatial passes
Reads symbols_autonamed_v10.txt → writes symbols_autonamed_v11.txt

New in v11 (research-verified yields):
  P14: RTTI vtable completor       — 957 vt* renames → enables cascade
  P12: Extended-transparency BFS   — 819 fn_8 (all generic prefixes transparent)
  P16: propose() collision bug fix — 19 fn_8 (extend retry 15→50)
  P15: Island BFS                  — ~80 conservative fn_8
  P13: String literal matching     — ~3 fn_8 (high confidence)
  P17: Spatial proximity           — ~150 conservative fn_8 (isolated only)

Math (Python-verified at design time):
  Conservative: +1,096 fn_8 → 91.49%
  Expected:     +1,140 fn_8 → 91.73%
  Optimistic:   +1,270 fn_8 → 92.54%
"""
...
SYMS     = os.path.join(BASE, 'config/434C4803/symbols_autonamed_v10.txt')
OUT      = os.path.join(BASE, 'config/434C4803/symbols_autonamed_v11.txt')
REPORT   = os.path.join(BASE, 'auto_namer_v11_report.txt')
...
with open(REPORT, 'w') as f:
    f.write(f"auto_namer_v11 Results\n{'='*70}\n\n")
