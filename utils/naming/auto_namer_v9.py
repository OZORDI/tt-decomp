#!/usr/bin/env python3
"""
auto_namer_v9.py — game_ semi-transparent BFS + manual bridge seeds + cascade
Reads symbols_autonamed_v8.txt → writes symbols_autonamed_v9.txt

New in v9:
  MANUAL_SEEDS     — 2 hand-analyzed bridge functions (game_90D0, game_C068)
                     seeded before any BFS runs, unlocking cascade chains
  P10 (NEW)        — game_ Semi-Transparent BFS:
                     treats game_XXXX nodes as pass-through in voting when
                     they have a dominant class in their 1-hop neighborhood.
                     Runs iteratively: naming a game_ unlocks more game_s.
  P11 (NEW)        — Dominant-caller threshold pass:
                     fn_8 with ≥5 callers all from same class → name it
  P4 re-run        — Re-runs full P4 BFS with the new seeds in place
"""
...
SYMS   = os.path.join(BASE, 'config/434C4803/symbols_autonamed_v8.txt')
OUT    = os.path.join(BASE, 'config/434C4803/symbols_autonamed_v9.txt')
REPORT = os.path.join(BASE, 'auto_namer_v9_report.txt')
RTTI   = os.path.join(BASE, 'rtti_vtable_map.json')

# ── Manual Seeds (hand-analyzed bridge functions) ─────────────────────────────
# These were identified by analysis of call neighborhoods and disassembly.
# game_90D0 (0x820F90D0): Cross-system registry/lookup called by 214 callers
#   across atSingleton, net, snSession, audBank, etc. Calls _locale_register
#   and two game_ helpers. Function signature matches atSingleton::Find or a
#   similar global lookup. Dominant 2-hop class = atSingleton (74 votes).
#   Best name: atSingleton_Find_90D0 (utility called by all singleton users)
#
# game_C068 (0x823EC068): 112-byte doubly-linked list insert, zero callees.
#   All 66 callers are snJoinMachine, snLeaveMachine, snMigrateMachine,
#   snCreateMachine, snSessionFinder — session state machine subsystem.
#   Disassembly: manipulates r3→[0], r3→[4], r3→[8] (head/tail/count of list)
#   and r4 (node). This is snSession::AddMember or snSession::EnqueueNode.
#   Best name: snSession_AddNode_C068
MANUAL_SEEDS = {
    0x820F90D0: ('atSingleton_Find_90D0',  'manual: 2-hop atSingleton(74) dominant, calls _locale_register, 214 callers across all classes'),
    0x823EC068: ('snSession_AddNode_C068', 'manual: 66 callers all snJoin/Leave/MigrateMachine, dlinked-list insert (r3=list r4=node)'),
}
...
# ── Symbol loading ────────────────────────────────────────────────────────────
print("\nLoading symbols (from v8 output)...")
sym_map   = {}
sym_lines = []
fn_meta   = {}
with open(SYMS) as f:
...
def get_class_clean(fn_va, extra_names=None):
    """Return meaningful class prefix for fn_va, or None."""
...
# ── PASS SEED: Apply Manual Bridge Seeds ─────────────────────────────────────
print("\n=== PASS SEED: Manual bridge seeds ===")
seed_count = 0
for addr, (name, reason) in MANUAL_SEEDS.items():
    if addr not in fn_set:
        ...
        continue
    nm = sym_map.get(addr, '')
    if not nm.startswith('game_') and not nm.startswith('fn_8'):
        print(f"  SKIP: 0x{addr:08X} already named '{nm}'")
        continue
    if propose(addr, name, reason, 'SEED', overwrite=True):
        # Extract class from name and seed fn_class
        parts = name.split('_')
        if parts:
...
# The core insight: game_XXXX functions that have a dominant class in their
# 1-hop neighborhood are "class-affiliated" even if we don't know the method.
# We treat them as pass-through nodes for voting, then name them with that class.
# This runs iteratively until convergence: each round's new names unlock more.
print("\n=== PASS 10: game_ semi-transparent BFS ===")
...
def bfs_vote_game_transparent(fn_va):
    """
    Vote for fn_va's class including game_ neighbors as pass-through.
    game_ nodes themselves contribute their neighbors' class votes
    (weighted by confidence that the game_ node belongs to that class).
    """
    cc = Counter()
    
    # Direct 1-hop named neighbors (bl calls + fn-ptr)
    for c in forward_cg.get(fn_va, set()):
        ...
        if cls:
            ...
            # Pass through: accumulate that game_'s neighborhood
...
for iteration in range(60):
    rc = 0
    # Process game_ functions (not just fn_8)
    ...
    for fn_va in targets:
        ...
        # For game_ nodes: lower threshold (they just need a dominant class signal)
        # For fn_8 nodes: normal threshold
        ...
        if score >= min_score and ratio >= min_ratio:
            ...
            if is_fn8:
                ...
                # For game_ renaming, append _g suffix to distinguish
                tag = 'P10r'
...
# fn_8 functions with ≥5 callers where ≥80% share the same class → name them.
# More aggressive than P4 but with a higher caller-count requirement.
...
with open(REPORT,'w') as f:
    f.write(f"auto_namer_v9 Results\n{'='*70}\n\n")
    f.write(f"Input:  symbols_autonamed_v8.txt\n")
    f.write(f"Output: symbols_autonamed_v9.txt\n\n")
    f.write(f"Manual seeds:\n")
    for addr, (name, reason) in MANUAL_SEEDS.items():
        f.write(f"  0x{addr:08X}  {name:<55s}  {reason}\n")
    f.write(f"\n")
    for tag in ['SEED','P10','P10r','P11','P4f','P4fw','P4rh','P6','P6r']:
