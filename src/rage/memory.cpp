...
// ─────────────────────────────────────────────────────────────────────────────
void datBase_ClearVtable(atSingleton *obj) { obj->vtable = nullptr; }
...
// Returns pointer to the value (at offset +4 in the entry) if found, nullptr
// otherwise.
//
...
const char *atSingleton::GetDebugName() {
  ...
  // Likely part of a longer error message like "Incorrect number of points
  // specified"
...
//
// This appears to check if the game is in a specific state (e.g., paused but
// not loading).
// ─────────────────────────────────────────────────────────────────────────────
...
//   +52..+60: Zeroed fields
// ─────────────────────────────────────────────────────────────────────────────
void fragDrawable_CopyData48(atSingleton *dest, void *param1,
                             void *sourceData) {
...
// ─────────────────────────────────────────────────────────────────────────────
void *crAnimation_LoadByName(atSingleton *loader, const char *animName,
  ...
  if (animation == nullptr) {
    ...
    if (animation == nullptr) {
      ...
      rage_DebugLog("Failed to load animation: %s from %s or %s", animName,
                    animName,
                    animName); /* UNVERIFIED — string not found in binary */
      return nullptr;
    }
  }
...
//
// Initializes three consecutive fields and calls another initialization
// function. Sets fields at offsets +14552, +14556, +14560 to specific values.
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
int32_t pgArray_FindActiveEntry(uint8_t *array, int32_t startOffset,
                                int32_t stride, uint8_t wrapFlag) {
...
// ─────────────────────────────────────────────────────────────────────────────
void netFirewall_TryAddByMaxCount(uint8_t *manager, uint32_t channelIdx,
                                  uint8_t *filter) {
  extern bool atSingleton_7068_fw(uint8_t *manager, uint32_t channelIdx,
                                  void *entry);
...
// ─────────────────────────────────────────────────────────────────────────────
void netFirewall_TryAddByDualRange(uint8_t *manager, uint32_t channelIdx,
                                   uint8_t *filter) {
  extern bool atSingleton_7068_fw(uint8_t *manager, uint32_t channelIdx,
                                  void *entry);
...
//   r5 = filter descriptor
// ─────────────────────────────────────────────────────────────────────────────
void netFirewall_TryAddByFrameRange(uint8_t *manager, uint32_t channelIdx,
                                    uint8_t *filter) {
  extern bool atSingleton_7068_fw(uint8_t *manager, uint32_t channelIdx,
                                  void *entry);
    ...
    for (uint32_t i = 0; i < maxEntries; i++) {
      if (registry[i] == nullptr) {
        ...
        if (registry[emptySlot] != nullptr) {
          ...
          rage_debugLog((const char *)0x8203F288);
          return (int32_t)emptySlot;
        }

        // Registration failed
        rage_debugLog((const char *)0x8203F2B4);
        return (int32_t)emptySlot;
      }
      emptySlot++;
    }
...
// ─────────────────────────────────────────────────────────────────────────────
void gdGameData_ClearLoadedFlag(atSingleton *obj) { *((uint8_t *)obj + 8) = 0; }
...
// ─────────────────────────────────────────────────────────────────────────────
const char *gdGameData_GetPropertyString() { return (const char *)0x8204E918; }
...
// ─────────────────────────────────────────────────────────────────────────────
void *atSingleton_GetTypeInfo() { return (void *)0x82029718; }
...
//   Always returns 0.
// ─────────────────────────────────────────────────────────────────────────────
uint32_t atSingleton_ReplaceSlotEntry(atSingleton *obj, void *nameBuffer,
                                      uint32_t slotIndex) {
  if (slotIndex > 3) {
    return 0;
  }
...
//   +0x3C  0
// ─────────────────────────────────────────────────────────────────────────────
void atSingleton_InitShapeFromSource(uint8_t *dest, const void *nameKey,
                                     const uint8_t *src) {
